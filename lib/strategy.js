...
Strategy.prototype.authenticate = function (req, options) {
  // The remember me cookie is only consumed if the request is not
  ...
  const isSigned = !!(this._opts !== undefined && this._opts.signed === true);
  const token = req.cookies.get(this._key, { signed: isSigned });
  ...
  function verified(err, user, info) {
    ...
    if (!user) {
      ...
      ctx.cookies.set(self._key, null, self._opts);
      return self.pass();
    }
    ...
    function issued(err, val) {
      ...
      ctx.cookies.set(self._key, val, self._opts);
      return self.success(user, info);
    }

    if (self._passReqToCallback) self._issue(req, user, issued);
    else self._issue(user, issued);
  }

  if (self._passReqToCallback) self._verify(req, token, verified);
  else self._verify(token, verified);
};
